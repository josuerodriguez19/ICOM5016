...
function Browse(categoryID){
	$.mobile.loading("show");
	$.ajax({
		url : "http://localhost:3412/ICOM5016Srv/categories/" + categoryID,
		method: 'get',
...
$(document).on('pagebeforeshow', "#browse_index", function( event, ui ) {
	console.log("Jose");
	$.ajax({
		url : "http://localhost:3412/ICOM5016Srv/categories",
		contentType: "application/json",
		success : function(data, textStatus, jqXHR){
			...
			for (var i=0; i < len; ++i){
				...
				if(category.mother == "empty")
				list.append("<li><a onclick=Browse(" + category.id + ")>" + category.name + "</a></li>");
			}
			list.listview("refresh");	
		},
		error: function(data, textStatus, jqXHR){
...
$(document).on('pagebeforeshow', "#product-page", function( event, ui ) {
	...
	$("#iPrice").val(selectedProduct.iPrice);
	$("#bPrice").val(selectedProduct.bPrice);
	$("#Description").val(selectedProduct.desc);
	$("#model").val(selectedProduct.model);
...
//alert("Entered Search");
$.ajax({
		url : "http://localhost:3412/ICOM5016Srv/movies",
		contentType: "application/json",
		success : function(data, textStatus, jqXHR){
			//alert("success");
			var productList = data.products;
